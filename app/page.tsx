    ...
    <section className="container py-8 md:py-12">
      <div className="grid md:grid-cols-2 gap-6 items-center">
        <div>
          ...
          <p className="text-neutral-400 mt-3 md:text-lg">Tap an island to load its weather & headlines instantly.</p>
          <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
            {ISLANDS.map(i=>(
              <button key={i.code} onClick={()=>setSelected(i)} className={`rounded-xl border px-3 py-2 text-sm transition ${selected.code===i.code?'bg-brand-600/20 border-brand-600':'bg-neutral-900 border-neutral-800 hover:bg-neutral-800'}`}>
                {i.name}
              </button>
            ))}
          </div>
          <a href={`/island/${selected.code}`} className="inline-block mt-4 text-sm underline">Open {selected.name} details →</a>
        </div>
