...
export async function GET(req: NextRequest) {
  ...
  if (!RAPID_KEY) { return Response.json({ ok:false, reason:'no_api_key', airports:list, flights:[] }) }
  const headers = { 'X-RapidAPI-Key': RAPID_KEY!, 'X-RapidAPI-Host': HOST }
  const now = new Date(); const end = new Date(now.getTime() + windowHours*3600*1000).toISOString()
  const results:any[] = []
  await Promise.all(list.slice(0,5).map(async a => { try { const r = await fetch(`https://${HOST}/airports/iata/${a.iata}/${dir}?fromUtc=${now.toISOString()}&toUtc=${end}`, { headers }); const data = await r.json(); results.push({ airport:a, data }) } catch {} }))
