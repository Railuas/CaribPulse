...
const now = Date.now();
const rnd = (n:number, spread:number) => +(n + (Math.random()-0.5)*spread).toFixed(1);

export const sampleHourly: Readonly<Hour[]> = Array.from({length:24}, (_,i)=>({
  t: now + i*3600_000,
  temp: rnd(29, 3),
...
export const sampleAlerts: Readonly<Alert[]> = [
...
export const sampleStorms: Readonly<Storm[]> = [
