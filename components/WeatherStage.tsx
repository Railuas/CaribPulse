...
import type { Point, Hour, Alert, Storm } from '../sample/fixtures';

function SparkInline({data, h=42, strokeWidth=2}:{data:ReadonlyArray<number>; h?:number; strokeWidth?:number}){
  ...
  return (
    <svg role="img" aria-label="spark" width={w} height={h} className="spark-inline">
...
export default function WeatherStage({
  ...
  const iframeSrc = useMemo(()=>{
    const p = new URLSearchParams({
      lat: String(point?.lat ?? 17.3), lon: String(point?.lon ?? -62.73), zoom: String(zoom),
      ...
      <div className="stage-body">
        ...
        {tab === 'hourly' && (
          <div className="panel panel-hourly" data-animate="fade">
            <div className="row">
              <div className="card" data-animate="float"><h4>Temperature (12h)</h4><SparkInline data={temps.slice(0,12)} /><div className="axis"><span>{Math.min(...temps)}°</span><span>{Math.max(...temps)}°</span></div></div>
              <div className="card" data-animate="float"><h4>Wind (12h)</h4><SparkInline data={winds.slice(0,12)} /><div className="axis"><span>{Math.min(...winds)} km/h</span><span>{Math.max(...winds)} km/h</span></div></div>
              <div className="card" data-animate="float"><h4>Precip (12h)</h4><SparkInline data={rains.slice(0,12)} /><div className="axis"><span>{Math.min(...rains)} mm</span><span>{Math.max(...rains)} mm</span></div></div>
            </div>
