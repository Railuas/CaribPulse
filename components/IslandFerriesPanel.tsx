...
type Props = { islandName: string };

export default function IslandFerriesPanel({ islandName }: Props){
  // map islandName -> API island code + labels + controls
  const cfg = useMemo(()=>{
    const name = islandName.toLowerCase();
    if (name.includes('kitts') || name.includes('nevis')){
      return { code: 'skn', dirA: 'Nevis → St. Kitts', dirB: 'St. Kitts → Nevis', controls: 'skn' as const };
    }
    if (name.includes('antigua') || name.includes('barbuda')){
      return { code: 'antigua', dirA: 'Antigua → Barbuda', dirB: 'Barbuda → Antigua', controls: 'simple' as const };
    }
    if (name.includes('trinidad') || name.includes('tobago')){
      return { code: 'tt', dirA: 'Port of Spain → Scarborough', dirB: 'Scarborough → Port of Spain', controls: 'simple' as const };
    }
    // fallback generic (no provider yet)
    return { code: 'unsupported', dirA: 'Outbound', dirB: 'Inbound', controls: 'none' as const };
  }, [islandName]);
  ...
  useEffect(()=>{ load(); /* initial and when controls change */ }, [cfg.code, type, day, source]);

  if (cfg.code === 'unsupported'){
    return <div className="card"><div className="muted small">No live ferry source yet for <strong>{islandName}</strong>. Tell me the operator/site you prefer and I’ll add it.</div></div>;
  }
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gap:12, gridTemplateColumns:'1fr 1fr' }}>
        <Table dir="a-to-b" title={cfg.dirA} />
        <Table dir="b-to-a" title={cfg.dirB} />
      </div>
      {data && (
