...
function pick(re: RegExp, xml: string): string | undefined {
...
function pickImage(xml: string): string | undefined {
  return (
    pick(/<media:content[^>]*url=["']([^"']+)["'][^>]*>/i, xml) ||
    pick(/<media:thumbnail[^>]*url=["']([^"']+)["'][^>]*>/i, xml) ||
    pick(/<enclosure[^>]*url=["']([^"']+)["'][^>]*>/i, xml) ||
    pick(/<img[^>]*src=["']([^"']+)["']/i, xml) ||
    undefined
  );
}

function text(tag: string, xml: string) {
